...
// MAGIC This notebook goes over basic graph analysis using [the GraphFrames package available on spark-packages.org](http://spark-packages.org/package/graphframes/graphframes). The goal of this notebook is to show you how to use GraphFrames to perform graph analysis. You're going to be doing this with Bay area bike share data from [Kaggle](https://www.kaggle.com/benhamner/sf-bay-area-bike-share/downloads/sf-bay-area-bike-share.zip). Please find the files in the Drive folder of the class.
// MAGIC 
...
val bikeStations = spark.read.format("csv").option("header", true).load("/FileStore/tables/station.csv")
val tripData = spark.read.format("csv").option("header", true).load("/FileStore/tables/trip.csv")
